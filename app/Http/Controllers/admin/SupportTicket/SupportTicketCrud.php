 ...
 */
class SupportTicketCrud extends Crud
{
	use TicketData;
	 ...
	 * @var string
	 */
	public $table_name 				= 'stm_support_tickets';

	public $model_name 				= 'App\Models\SupportTicket\SupportTicket';
	/**
    ...
    {
		//$this->page_title = 'Support Ticket List';
		//$this->setAddLink(route('admin-support-ticket-management-add-ticket'));
    	//$data = $this->rendarShow();
		//return view('admin.crud.show',$data);
		$user_id = $this->user->id;
		$this->setLeftSideBarData();
		$add_button = route('admin-support-ticket-management-add-ticket');

		$ticket_status_type = StStatusType::where('status','active')->get()->toArray();
		$support_ticket = SupportTicket::get();
		...
		foreach($support_ticket as $support_ticket_data)
		{
			...
			$StMessages = StMessage::where('ticket_id',$support_ticket_data->id)->where('sender','!=',$user_id)->where('state','not seen')->get();
		...
		return view('admin/support-ticket/admin_support_ticket_list',['add_button' => $add_button,'support_ticket' => $totalData,'ticket_status_type' => $ticket_status_type,'profile_image' => $this->profile_image, 'parent_menu' => $this->parent_menu, 'sub_menu' =>$this->sub_menu, 'page_title' => 'All Ticket']);
    }
	/**
    ...
    public function getAddTicket()
    {
        ...
        return view('admin/support-ticket/admin_add_ticket',['department' => $department, 'ticketType' => $ticketType,  'priority' => $priority, 'statusType' => $statusType,  'profile_image' => $this->profile_image, 'parent_menu' => $this->parent_menu, 'sub_menu' =>$this->sub_menu, 'page_title' => 'Add Ticket']);
    }
	...
	public function getEditTicket($id)
	{
		...
		$ticket_data = DB::table('stm_st_messages')
      ->where('stm_support_tickets.id',$id)
			->groupBy('stm_support_tickets.id')
    ...
    $users = checkParentIds($player_id); //checkParentIds defined in helpers
        ...
        return view('admin/support-ticket/admin_edit_ticket',['player' => $player, 'ticketData' => $ticket_data,'allUsers' => $users,'department' => $department, 'ticketType' => $ticketType,  'priority' => $priority, 'statusType' => $statusType,  'profile_image' => $this->profile_image, 'parent_menu' => $this->parent_menu, 'sub_menu' =>$this->sub_menu, 'page_title' => 'Edit Ticket']);
	...
	public function callDefault()
	{
		...
		$where = "WHERE stm_support_tickets.deleted_at is null";
