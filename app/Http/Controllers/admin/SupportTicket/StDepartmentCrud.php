...
use App\Http\Requests\StDepartmentRequest;
use App\Http\Traits\DataSaver;

/**
 *  This is a command to fetch all support ticket departments and save into database from admin panel.
 *
 *  @author	Sourav Chowdhury
 */
class StDepartmentCrud extends Crud
{
	use DataSaver;
	/**
	 ...
	 * @var string
	 */
	public $table_name 				= 'stm_st_departments';

	public $model_name = 'App\Models\SupportTicket\StDepartment';
	/**
	 * route name that shold be used to create different action link. REQUIRED
	 * @var string
	 */
	public $route_slug 				= 'admin-support-ticket-management-ticket-department-';
	/**
    ...
    {
		$this->page_title = 'Ticket Department List';
	...
	{
		$this->page_title = 'View Ticket Department';
	...
	public function add()
	{
		$this->page_title = 'Add Ticket Department';
		$data = $this->rendarAdd();
		return view('admin.crud.form',$data);
	}
	/**
	 * This will insert data into databse
	 * @param  StDepartmentRequest $request validated form request
	 * @return void                 redirect page
	 */
	public function insert(StDepartmentRequest $request)
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Department Added From Admin',$user_id);
	...
	{
		$this->page_title = 'Edit Ticket Department';
	 ...
	 * @return void                 redirect page
	 */
	public function update(StDepartmentRequest $request)
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Department Updated From Admin',$user_id);
	...
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Department Deleted From Admin',$user_id);
	...
	public function callDefault()
	{
		...
		$where = "WHERE stm_st_departments.deleted_at is null";
