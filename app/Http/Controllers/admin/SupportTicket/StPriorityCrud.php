...
use App\Http\Requests\StPriorityRequest;
use App\Http\Traits\DataSaver;

/**
 *  This is a command to fetch all support ticket priorities and save into database from admin panel.
 *
 *  @author	Sourav Chowdhury
 */
class StPriorityCrud extends Crud
{
	use DataSaver;
	/**
	 ...
	 * @var string
	 */
	public $table_name 				= 'stm_st_priorities';

	public $model_name 				= 'App\Models\SupportTicket\StPriority';
	/**
	 * route name that shold be used to create different action link. REQUIRED
	 * @var string
	 */
	public $route_slug 				= 'admin-support-ticket-management-ticket-priority-';
	/**
    ...
    {
		$this->page_title = 'Ticket Priority List';
	...
	{
		$this->page_title = 'View Ticket Priority';
	...
	public function add()
	{
		$this->page_title = 'Add Ticket Priority';
		$data = $this->rendarAdd();
		return view('admin.crud.form',$data);
	}
	/**
	 * This will insert data into databse
	 * @param  StPriorityRequest $request validated form request
	 * @return void                 redirect page
	 */
	public function insert(StPriorityRequest $request)
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Priority Added From Admin',$user_id);
	...
	{
		$this->page_title = 'Edit Ticket Priority';
	 ...
	 * @return void                 redirect page
	 */
	public function update(StPriorityRequest $request)
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Priority Updated From Admin',$user_id);
	...
	{
		$user_id = $this->user->id;
		$this->log('Support Ticket Priority Deleted From Admin',$user_id);
	...
	public function callDefault()
	{
		...
		$where = "WHERE stm_st_priorities.deleted_at is null";
