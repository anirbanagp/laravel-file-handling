...
class MailTemplateCrud extends Crud
{
	use DataSaver;
	/**
	 ...
	 * @var string
	 */
	public $route_slug 				= 'admin-settings-mail-template-';
	/**
    ...
    {
		$this->page_title = 'Mail Template List';
	...
	{
		$this->page_title = 'View Mail Template';
	...
	public function add()
	{
		$this->page_title = 'Add Mail Template';
		$data = $this->rendarAdd();
		return view('admin.crud.form',$data);
	}
	/**
	 * This will insert data into databse
	 * @param  MailTemplateRequest $request validated form request
	 * @return void                 redirect page
	 */
	public function insert(MailTemplateRequest $request)
	{
		$user_id = Session::get('user_id');
		$this->log('Mail Template Added From Admin',$user_id);
	...
	{
		$this->page_title = 'Edit Mail Template';
	...
	{
		$user_id = Session::get('user_id');
		$this->log('Mail Template Updated From Admin', null, (string)MailTemplate::find($request->id));
	...
	{
		$user_id = Session::get('user_id');
		$this->log('Mail Template Deleted From Admin',$user_id);
